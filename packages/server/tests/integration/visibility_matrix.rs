...
mod contest_problem_list_matrix {
    ...
    async fn anonymous_rejected_by_mandatory_auth_before_activation() {
        ...
        let res = app
            .get_without_token(&routes::contest_problems(contest_id))
            .await;
...
mod contest_detail {
    ...
    async fn setup() -> Fixture {
        ...
        let participant = app
            ...
            .await;
        let contest_id =
            create_contest_window(&app, &admin, "Contest Detail Fixture", false, INSIDE_WINDOW)
                .await;
        enroll_participant(&app, &admin, contest_id, &participant).await;
        Fixture {
            app,
    ...
    async fn anonymous_is_401() {
        ...
        let res = f
            .app
            .get_without_token(&routes::contest(f.contest_id))
            .await;
        assert_eq!(res.status, 401);
        assert_eq!(res.body["code"], "TOKEN_MISSING");
    }
    ...
    async fn window_gate_isolated_public_contest_before_activation_is_404() {
        let (app, non_participant, contest_id) = setup_public(BEFORE_ACTIVATION).await;
        let res = app
            .get_with_token(&routes::contest(contest_id), &non_participant)
            .await;
        assert_eq!(res.status, 404);
        assert_eq!(res.body["code"], "NOT_FOUND");
    }
...
mod contest_problem_detail {
    ...
    async fn setup() -> Fixture {
        ...
        let participant = app
            ...
            .await;
        let contest_id =
            create_contest_window(&app, &admin, "Problem Detail Fixture", false, INSIDE_WINDOW)
                .await;
    ...
    async fn anonymous_is_401() {
        ...
        let res = f
            .app
            .get_without_token(&routes::problem(f.problem_id))
            .await;
        assert_eq!(res.status, 401);
        assert_eq!(res.body["code"], "TOKEN_MISSING");
    }
...
mod problem_attachment_list {
    ...
    async fn setup() -> Fixture {
        ...
        let problem_id = app
            .create_hidden_problem(&admin, "Attachment Problem")
            .await;
        app.add_problem_to_contest(contest_id, problem_id, &admin)
            .await;
        let upload = app
    ...
    async fn anonymous_is_401() {
        ...
        let res = f
            .app
            .get_without_token(&routes::attachments(f.problem_id))
            .await;
        assert_eq!(res.status, 401);
        assert_eq!(res.body["code"], "TOKEN_MISSING");
    }
...
mod contest_submission_list {
    ...
    async fn setup() -> Fixture {
        ...
        let participant = app
            .create_authenticated_user("participant", "pass1234")
            .await;
        let contest_id = create_contest_window(
            &app,
            &admin,
            "Submission List Fixture",
            false,
            INSIDE_WINDOW,
        )
        .await;
    ...
    async fn non_participant_is_404() {
        ...
        let res = f
            .app
            .get_with_token(
                &routes::contest_submissions(f.contest_id),
                &f.non_participant,
            )
            .await;
        assert_eq!(res.status, 404);
...
mod submission_detail {
    ...
    async fn setup() -> Fixture {
        ...
        let contest_id = create_contest_window(
            &app,
            &admin,
            "Submission Detail Fixture",
            false,
            INSIDE_WINDOW,
        )
        .await;
        let problem_id = app
            .create_problem(&admin, "Submission Detail Problem")
            .await;
        app.add_problem_to_contest(contest_id, problem_id, &admin)
            .await;
    ...
    async fn anonymous_is_401() {
        ...
        let res = f
            .app
            .get_without_token(&routes::submission(f.submission_id))
            .await;
        assert_eq!(res.status, 401);
        assert_eq!(res.body["code"], "TOKEN_MISSING");
    }
...
mod soft_delete {
    ...
    async fn contest_soft_deleted_is_not_found_even_for_contest_manage_admin() {
        ...
        let res = app
            .get_with_token(&routes::contest(contest_id), &admin)
            .await;
        assert_eq!(res.status, 404);
        assert_eq!(res.body["code"], "NOT_FOUND");
    }
    ...
    async fn problem_soft_deleted_is_not_found_even_for_problem_edit_admin() {
        ...
        let delete_res = app
            .delete_with_token(&routes::problem(problem_id), &admin)
            .await;
        assert_eq!(delete_res.status, 204);
    ...
    async fn submission_owner_can_still_view_own_submission_after_contest_soft_deleted() {
        ...
        let contest_id = create_contest_window(
            &app,
            &admin,
            "Doomed Submission Contest",
            false,
            INSIDE_WINDOW,
        )
        .await;
        let problem_id = app
            .create_problem(&admin, "Doomed Submission Problem")
            .await;
        app.add_problem_to_contest(contest_id, problem_id, &admin)
            .await;
        enroll_participant(&app, &admin, contest_id, &owner).await;
