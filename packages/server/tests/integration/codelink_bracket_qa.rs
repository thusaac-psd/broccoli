...
//! `codelink_bracket.rs`'s single linear end-to-end flow already proves
...
/// `codelink_bracket.rs` relies on, see its own comment on `setup.rs`'s
...
// started contest) and NEVER reach the codelink-bracket plugin's
// phase/ownership-aware `decide_visibility`:
// `plugins/codelink-bracket/src/visibility.rs`'s `"problem"` arm explicitly
...
#[ignore = "DEFECT (host-side residual, confirmed separate from the reachability leak this \
            ...
            NOT_YOUR_PROBLEM sanity check below (and likely codelink_bracket.rs's own gate-probe \
...
async fn defect_standalone_submission_route_bypasses_bracket_gate_entirely() {
    ...
    // `codelink_bracket.rs`'s "Gate probe (a)".
    let res = submit(
        &fx.app,
        fx.contest_id,
...
/// New, not covered by `codelink_bracket.rs`: submitting during the
...
/// `codelink_bracket.rs`'s two gate probes. Once B has ranked A's group
...
/// New, not covered by `codelink_bracket.rs`: an eliminated player (B, who
...
/// SAME architectural asymmetry `codelink_bracket.rs`'s "Gate probe (b)"
...
async fn eliminated_player_cannot_submit_to_a_round_they_have_no_match_in() {
    ...
    // match (id 8, A vs C -- see `codelink_bracket.rs`'s comment on
...
/// New, not covered by `codelink_bracket.rs`: a user who registered for the
...
//
// `codelink_bracket.rs` already confirms the "sees everything" half; this
