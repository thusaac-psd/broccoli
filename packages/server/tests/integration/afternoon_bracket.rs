...
//!
//! Every other afternoon-bracket test (112 of them, in
//! `plugins/afternoon-bracket/src/**`) calls `decide_problem`,
...
//! (`plugins/afternoon-bracket/src/routes.rs::mask_group`) is a SEPARATE,
...
//! and a merged `plugins_dir` combining the real `afternoon-bracket`
...
//! invoked), and the test-only `afternoon-judge-fixture` evaluator (stands
...
/// afternoon-bracket) - NOT a recursive directory copy, which would also
...
fn merged_plugins_dir() -> tempfile::TempDir {
    ...
    copy_plugin(
        workspace_root.join("plugins/afternoon-bracket"),
        "afternoon-bracket",
        "afternoon_bracket.wasm",
    );
    copy_plugin(
    ...
    copy_plugin(
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/afternoon-judge"),
        "afternoon-judge-fixture",
        "afternoon_judge_fixture.wasm",
    );
...
/// Create a real problem backed by the `afternoon-judge-fixture` evaluator,
    ...
    let res = app
        .post_with_token(
            ...
            &json!({
                ...
                "time_limit": 1000,
                "memory_limit": 262144,
                "problem_type": "afternoon-judge-fixture",
                "checker_format": "none",
                "default_contest_type": "afternoon-bracket",
                "is_public": true,
            }),
            staff_token,
        )
...
#[tokio::test]
async fn afternoon_bracket_end_to_end_through_the_real_host() {
