...
export function formatCountdownLabel(
  ...
  switch (status.kind) {
    case 'no-data':
      return t('codelink-bracket.countdown.noData');
    case 'counting':
      return t('codelink-bracket.countdown.counting', {
        time: formatDurationMs(status.remainingMs),
      });
    case 'waiting-for-server':
      return t('codelink-bracket.countdown.waitingForServer', {
        time: formatDurationMs(status.overdueMs),
      });
    case 'decided':
      return t('codelink-bracket.countdown.decided');
  }
}
