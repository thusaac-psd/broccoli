 ...
 * (contest_type = "codelink-bracket", position = "prepend"). Shows this
 ...
 */
export function BracketLiveXiaoju({
  contestId,
  problemId,
  submissions,
}: LiveXiaojuViewProps) {
    ...
    <div className="mb-4 rounded-md border border-border p-3">
      <div className="mb-1 flex items-center justify-between">
        <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {t('codelink-bracket.live.title', { status: t(status.labelKey) })}
        </h4>
      ...
      {match.state === 'awaiting_judge' && (
        <p className="text-xs text-amber-700">
          {t('codelink-bracket.live.awaitingJudge')}
        </p>
      )}
